  ...
  const versions = [
    {
      ...
      sections: [
        ...
        { fr:'Gouvernance Figma', en:'Figma governance', items:[
          {fr:'Chantier démarré : sécurité d\'écriture Tokens Studio → GitHub. Branche <code>agentica/proposals</code> créée (depuis <code>main</code>) comme seule destination d\'écriture autorisée pour un futur credential Tokens Studio ; la protection déjà en place sur <code>main</code> (ADR-076/077) couvre déjà l\'exigence "PR + refus des push directs" sans configuration supplémentaire (ADR-078). Restent deux actions humaines : compte de service GitHub dédié + PAT scoped, et pointage de la sync de lecture Tokens Studio sur <code>main</code> dans le panneau du plugin Figma',en:'Initiative started: Tokens Studio → GitHub write-access safety. <code>agentica/proposals</code> branch created (off <code>main</code>) as the sole allowed write destination for a future Tokens Studio credential; the protection already in place on <code>main</code> (ADR-076/077) already covers the "PR + no direct pushes" requirement with no extra configuration (ADR-078). Two human-only actions remain: a dedicated GitHub service account + scoped PAT, and pointing Tokens Studio\'s read sync at <code>main</code> in the Figma plugin panel'},
        ]},
      ],
    },
    {
